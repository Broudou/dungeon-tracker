<script>
  ...
  const LORE_CATS   = ['world','faction','npc','location','custom'];
  ...
  function openEditLore(entry, e) {
    e?.stopPropagation();
    expandedLoreId = entry._id; editingLore = entry;
    loreForm = JSON.parse(JSON.stringify(entry)); newLoreOpen = false;
  }

  function closeLore() { expandedLoreId = null; editingLore = null; newLoreOpen = false; }

  function openNewLore() {
    expandedLoreId = null; editingLore = null;
    loreForm = freshLore(); newLoreOpen = true;
  }
    ...
    try {
      ...
      if (editingLore) {
        res  = await fetch(`/api/campaigns/${id}/lore/${editingLore._id}`, {
          method: 'PATCH', credentials: 'include',
          headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(loreForm),
        });
        data = await res.json();
        if (!res.ok) throw new Error(data.message);
  ...
  async function deleteLore(entryId, e) {
    ...
    try {
      const res = await fetch(`/api/campaigns/${id}/lore/${entryId}`, { method: 'DELETE', credentials: 'include' });
      if (!res.ok) throw new Error((await res.json()).message);
...
<div class="page">
  <div class="container">
    ...
    {:else}
      ...
      {#if activeTab === 'lore'}
        <div class="section-header">
          <span class="section-title">Lore Entries</span>
          <button class="btn btn-primary btn-sm" on:click={openNewLore}>+ Add Entry</button>
        </div>

        {#if newLoreOpen}
          <div class="card mb-md">
            <h3 style="font-size: 0.875rem; font-weight: 600; margin-bottom: 1rem;">New Lore Entry</h3>
            <div class="field"><label>Title *</label><input bind:value={loreForm.title} /></div>
            <div class="field">
              <label>Category</label>
              <select bind:value={loreForm.category}>
                {#each LORE_CATS as cat}<option value={cat}>{cat}</option>{/each}
              </select>
            </div>
            <div class="field"><label>Content</label><textarea bind:value={loreForm.body} rows="5"></textarea></div>
            <div class="field">
              <label class="checkbox-label">
                <input type="checkbox" bind:checked={loreForm.visibleToPlayers} />
                Visible to players
              </label>
            </div>
        ...
        {#if campaign.lore.length === 0 && !newLoreOpen}
          <div class="empty-card">No lore entries yet.</div>
        {:else}
          <div class="list">
            {#each campaign.lore as entry (entry._id)}
              <div
                ...
                role="button" tabindex="0"
                on:keydown={e => e.key === 'Enter' && openEditLore(entry)}
              >
                <div class="item-row-header">
                  <div>
                    <div class="flex-center gap-xs mb-sm">
                      <span class="player-name">{entry.title}</span>
                      <span class="badge">{entry.category}</span>
                      {#if entry.visibleToPlayers}<span class="badge">Public</span>{/if}
                    </div>
                    {#if entry.body}
                      <p class="text-muted text-sm truncate" style="max-width: 500px;">{entry.body}</p>
                    {/if}
                  </div>
                  <span class="item-row-hint">Edit</span>
                </div>
                ...
                {#if expandedLoreId === entry._id}
                  <div class="expand-form" on:click|stopPropagation role="presentation">
                    ...
                    <div class="flex-center gap-sm mt-md">
                      <button class="btn btn-primary btn-sm" on:click={saveLore} disabled={saving}>
                        {saving ? 'Saving…' : 'Save'}
                      </button>
